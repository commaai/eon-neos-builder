...
azcopy cp --overwrite=false $NEOSUPDATE/recovery-$RECOVERY_HASH.img "https://$DATA_ACCOUNT.blob.core.windows.net/$DATA_CONTAINER/recovery-$RECOVERY_HASH.img?$DATA_SAS_TOKEN"
azcopy cp --overwrite=false $NEOSUPDATE/ota-signed-$OTA_HASH.zip "https://$DATA_ACCOUNT.blob.core.windows.net/$DATA_CONTAINER/ota-signed-$OTA_HASH.zip?$DATA_SAS_TOKEN"
