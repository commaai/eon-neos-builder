...
make DTC_EXT=$DTC DTC_OVERLAY_TEST_EXT=$UFDT CONFIG_BUILD_ARM64_DT_OVERLAY=y tici_defconfig
...
android/out/host/linux-x86/bin/mkbootimg \
  ...
  --cmdline "console=ttyMSM0,115200n8 earlycon=msm_geni_serial,0xA84000 androidboot.hardware=qcom androidboot.console=ttyMSM0 androidboot.selinux=permissive video=vfb:640x480-32@30 vfb.videomemorysize=3145728 msm_rtb.filter=0x237 ehci-hcd.park=3 lpm_levels.sleep_disabled=1 service_locator.enable=1 swiotlb=2048 androidboot.configfs=true androidboot.usbcontroller=a600000.dwc3 buildvariant=userdebug" \
  --base 0x80000000 \
...
# the key to making keymaster happy is *not* signing, but adding the hash.
